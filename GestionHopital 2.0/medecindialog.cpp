...
void MedecinDialog::chargerMedecins() {
    ...
    ui->tableWidgetMedecins->setRowCount(0);  // Nettoyer la table
    ui->tableWidgetMedecins->setColumnCount(6);  // Nombre de colonnes à afficher

    // Définir les en-têtes de colonne
    QStringList headers = {"ID", "Nom", "Prénom", "Spécialité", "Téléphone", "Email"};
    ui->tableWidgetMedecins->setHorizontalHeaderLabels(headers);
...
void MedecinDialog::modifierMedecin() {
    ...
    int id_medecin = ui->tableWidgetMedecins->item(row, 0)->text().toInt();
    QString nom = ui->lineEditNom->text();
    QString prenom = ui->lineEditPrenom->text();
    QString specialite = ui->lineEditSpecialite->text();
    QString telephone = ui->lineEditTelephone->text();
    ...
    QSqlQuery query;
    query.prepare("UPDATE Medecins SET nom = :nom, prenom = :prenom, specialite = :specialite, "
                  "telephone = :telephone, email = :email WHERE id_medecin = :id");
...
void MedecinDialog::supprimerMedecin() {
    ...
    int id = ui->tableWidgetMedecins->item(row, 0)->text().toInt();
    QString nom = ui->tableWidgetMedecins->item(row, 1)->text();
    QString prenom = ui->tableWidgetMedecins->item(row, 2)->text();
    ...
    confirmation = QMessageBox::question(
        ...
        QString("Voulez-vous vraiment supprimer le médecin %1 %2 ?").arg(nom).arg(prenom),
